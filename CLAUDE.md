...
A separate static page implementing the 0014 forecast model (DC/PCV/**loadcell**, house-driven). Distinct from `index.html` (per-camera-per-month): driven by a single `+houses` slider over a fixed現況 baseline (DC 406 / PCV 247 / loadcell 406). Five core cost items: EC2 realtime, S3, MSK (stepped), MongoDB (fsUsed-stepped), AWS IoT Core. MongoDB uses a single tier model — M50 (≤4 TB) → M50 + Extended (4–8 TB) → M80/sharding (>8 TB) — per **CC-770**, which confirmed M50 supports Extended Storage to 8 TB (so the doc's old Version A/M60 branch is obsolete). Also has a **per-camera margin section** (`calcMargin()`, 0014 §3): marginal monthly cost of one DC camera (incl. its loadcell, ~$37.8) vs one PCV camera (~$4.0) — the boss-facing "cost to serve one more camera". Calc logic is pure functions between `/* CALC-START */ … /* CALC-END */`; `verify.mjs` (`node verify.mjs`) asserts them against 0014's anchor values. Source of truth: `../calyx-infra/clusters/oregon-production/implementation-plan/0014-pipeline-cost-re-evaluation.md`.
