...
工具 SHALL 在 per-camera 月費區塊之後、艦隊總成本區塊之前，顯示一個獨立的「共享基礎設施月費」區塊，列出 MSK 與 MongoDB 的**總月費**（整個部署，非單台分攤）。MSK 行下方 SHALL 嵌入 MSK 容量規劃子區塊（詳見 `msk-capacity-planner` spec）。
