...
工具 SHALL 對每個批次獨立套用現有 EC2 與 S3 IT 公式，計算該批次的 EC2 與 S3 月費小計。
...
#### Scenario: 批次 S3 IT 成本計算
- **WHEN** 某批次的上線月數為 n，含 mDC 台 DC 相機與 mPCV 台 PCV 相機
- **THEN** 批次 S3 費用 = calcS3IT_DC(n) × mDC + calcS3IT_PCV(n) × mPCV（USD/月）
...
工具 SHALL 顯示艦隊當月總費用，並拆解為 EC2 合計、S3 IT 合計、MSK、MongoDB 四項。

#### Scenario: 艦隊總費用計算
- **WHEN** 使用者設定至少一個批次
- **THEN** Fleet Total = Σ(批次 EC2) + Σ(批次 S3 IT) + MSK_fleet + MongoDB_fleet（USD/月）

#### Scenario: 數值驗證（單批次基準）
- **WHEN** 單一批次 N=12, M_DC=340, M_PCV=240
- **THEN** 批次 EC2 = $3,838.32（DC $2,788.56 + PCV $1,049.76）；批次 S3 = $17,088.80（DC $44.96×340 + PCV $7.51×240）；MSK_fleet = $1,238.88；MongoDB_fleet = $6,019.00；Fleet Total = $28,185.00

#### Scenario: 各項拆解顯示
- **WHEN** 艦隊總費用計算完成
- **THEN** 結果區塊分別顯示：EC2 合計、S3 IT 合計、MSK、MongoDB 各項金額，以及 Fleet Total
